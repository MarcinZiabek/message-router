...
Novadays, no one considers implementing the communication layer using pure sockets. Some messaging libraries are developed such as ZeroMQ and its .NET Framework port called NetMQ. Number of probles solved by them is enormous: sending information over newtork in a lightweight manner, making sure that a whole message is received or distrubuting messages across many subscribers. In the real word scenarios, it is not enough. 

Let's consider how alternatives look like. The HTTP protocol provides different endpoints - every message is appropriately  deserialized and routed to methods in your code. The messaging system in ZeroMQ does not contain any header information and, therefore, does not provide any similar functinality.

The NetmqRouter acts as an additional layer of abstraction between your code and communication layer. In the internal implentation, every message consists of two message parts: a header (containing name of the communication route aka address) and a message body.
